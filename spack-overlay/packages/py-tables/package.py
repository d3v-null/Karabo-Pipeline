...
class PyTables(PythonPackage):
    ...
    # numpy 2 compat: tables 3.9.2 supports numpy 2 (Python 3.10 ok)
    version("3.9.2", sha256="d470263c2e50c4b7c8635a0d99ac1ff2f9e704c24d71e5fa33c4529e7d0ad9c3")
    ...
    depends_on("py-cython@0.29.32:", when="@3.9:", type=("build", "run"))
    ...
    # numpy 2 compat: tables 3.9.2+ supports numpy 2; only cap @:3.9.1
    depends_on("py-numpy@:1", type=("build", "run"), when="@:3.9.1")
    ...
    def patch(self):
        # NumPy 2 changed npy_complexXX from structs with .real/.imag members
        # to C99 _Complex types, so HOFFSET(npy_complexXX, real/imag) no
        # longer compiles. Replace with hardcoded ABI-stable offsets (real
        # part is always at offset 0, imaginary at sizeof(base type)).
        # Fix backported from PyTables commit b05581af ("Compatibility with
        # numpy 2.0"), included in 3.10.0+.
        if self.spec.satisfies("@3.9.2 ^py-numpy@2:"):
            for src_file in ("src/utils.c",):
                for old, new in (
                    ("HOFFSET(npy_complex64, real)", "0"),
                    ("HOFFSET(npy_complex64, imag)", "4"),
                    ("HOFFSET(npy_complex128, real)", "0"),
                    ("HOFFSET(npy_complex128, imag)", "8"),
                    ("HOFFSET(npy_complex192, real)", "0"),
                    ("HOFFSET(npy_complex192, imag)", "12"),
                    ("HOFFSET(npy_complex256, real)", "0"),
                    ("HOFFSET(npy_complex256, imag)", "16"),
                ):
                    filter_file(old, new, src_file, string=True)
