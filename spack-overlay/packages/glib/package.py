...
class MesonBuilder(meson.MesonBuilder):
    ...
    def meson_args(self):
        ...
        # Work around BuildKit/Docker sandbox restrictions that can break
        # glib's test resource generation with EPERM (e.g. "Failed to close file
        # descriptor for child process (Operation not permitted)").
        #
        # This only affects glib's own test suite; it does not disable any
        # Karabo-Pipeline tests in the Dockerfile.
        if self.spec.satisfies("@2.74:"):
            args.append("-Dtests=false")
            args.append("-Dinstalled_tests=false")
